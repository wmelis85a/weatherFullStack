...
class LegacyMetadata:
    ...
    def check(self, strict=False):
        ...
        if strict and missing != []:
            msg = "missing required metadata: {}".format(", ".join(missing))
...
class Metadata:
    ...
    GENERATOR = f"distlib ({__version__})"
            ...
            if (scheme or self.scheme) not in exclusions:
                ...
                if not m:
                    raise MetadataInvalidError(
                        f"'{value}' is an invalid value for the '{key}' property"
                    )
        ...
        for key, exclusions in self.MANDATORY_KEYS.items():
            if key not in mapping:
                if scheme not in exclusions:
                    missing.append(key)
        if missing:
            msg = "Missing metadata items: {}".format(", ".join(missing))
            raise MetadataMissingError(msg)
