...
class Locator:
    ...
    def locate(self, requirement, prereleases=False):
        ...
        if r is None:  # pragma: no cover
            raise DistlibException(f"Not a valid requirement: {requirement!r}")
...
""",
    ...
    def links(self):
        ...
        for match in self._href.finditer(self.data):
            ...
            url = unescape(url)
            url = self._clean_re.sub(lambda m: f"%{ord(m.group(0)):2x}", url)
            result.add((url, rel))
...
class DirectoryLocator(Locator):
    ...
    def __init__(self, path, **kwargs):
        ...
        if not os.path.isdir(path):  # pragma: no cover
            raise DistlibException(f"Not a directory: {path!r}")
        self.base_dir = path
...
class DependencyFinder:
    ...
    def find(self, requirement, meta_extras=None, prereleases=False):
        ...
        if isinstance(requirement, Distribution):
            ...
            if dist is None:
                raise DistlibException(f"Unable to locate {requirement!r}")
