...
class PackageIndex:
    ...
    def sign_file(
        ...
        if rc != 0:
            raise DistlibException(f"sign command failed with error code {rc}")
        return sig_file
    ...
    def upload_documentation(self, metadata, doc_dir):  # pragma: no cover
        ...
        if not os.path.isdir(doc_dir):
            raise DistlibException(f"not a directory: {doc_dir!r}")
        fn = os.path.join(doc_dir, "index.html")
        if not os.path.exists(fn):
            raise DistlibException(f"not found: {fn!r}")
        metadata.validate()
    ...
    def verify_signature(self, signature_filename, data_filename, keystore=None):
        ...
        if rc not in (0, 1):
            raise DistlibException(f"verify command failed with error code {rc}")
        return rc == 0
    ...
    def download_file(self, url, destfile, digest=None, reporthook=None):
        ...
        if digester:
            ...
            if digest != actual:
                raise DistlibException(
                    f"{hasher} digest mismatch for {destfile}: expected "
                    f"{digest}, got {actual}"
                )
