...
const ForecastCardExtended: React.FC<ForecastCardExtendedProps> = ({
  city,
}) => {
    ...
    async function fetchWeatherData() {
      ...
      try {
        ...
        const response = await axios.get<WeatherData>(
          `${import.meta.env.VITE_API_EXTENDED_CONDITIONS_URL}?city=${city}`,
        );
    ...
    <div className="max-w-4xl mx-auto p-6 font-sans">
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <p className="text-2xl font-semibold text-center mb-4">
          Condição:{" "}
          <strong className="text-blue-600">{weatherData.condition}</strong>
        </p>
        ...
        <div className="flex justify-around items-center text-xl text-gray-700">
          <span>
            ...
            <span className="font-bold text-red-500">
              {weatherData.min_temp_c}°C
            </span>
          </span>
          <span>
            Máxima:{" "}
            <span className="font-bold text-blue-500">
              {weatherData.max_temp_c}°C
            </span>
          </span>
        </div>
      </div>
        ...
        {weatherData.hourly.map((hour: HourlyForecast, index: number) => (
          ...
          >
            <p className="text-lg font-bold text-gray-900">
              {hour.time.split(" ")[1].slice(0, 5)}h
            </p>
            <p className="text-3xl font-light text-gray-800 my-2">
              {hour.temp_c}°C
            </p>
