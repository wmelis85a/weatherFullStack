...
export async function getHomeForecast(
  query: string,
...
export async function getDetailedConditions(
  query: string,
): Promise<DetailedWeatherData> {
  try {
    console.log("🔎 Enviando query para detailed:", query);

    const response = await api.get<DetailedWeatherData>(
      `${VITE_API_CONDITIONS_URL}`,
      {
        params: { city: query },
      },
